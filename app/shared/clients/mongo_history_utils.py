...
class HistoryMongoTool:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.timeout_ms = int(os.getenv("MONGO_SERVER_SELECTION_TIMEOUT_MS", "3000"))

            # 创建MongoDB客户端实例，建立与数据库的连接
            self.client = MongoClient(self.mongo_url, serverSelectionTimeoutMS=self.timeout_ms)
            # 获取指定名称的数据库对象 user 库
...
def get_recent_messages(session_id: str, limit: int = 10) -> list[dict[str, Any]]:
    ...
    try:
        ...
        # db.chat_message.find({session_id:session_id}).sort({ts:-1}).skip(5).limit(limit)
