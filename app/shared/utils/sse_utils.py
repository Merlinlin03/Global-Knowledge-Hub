...
class SSEEvent:
    READY = "ready"         # 连接建立
    PROGRESS = "progress"   # 任务节点进度  节点 完成运行 状态
    DELTA = "delta"         # LLM 流式输出增量  流式返回字符串
    FINAL = "final"         # 最终完整答案  流式最终图片覆盖
    ERROR = "error"         # 错误信息 任务报错
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                # stream_queue.get(block=True,timeout=5.0)
